...
where
    T: TemperatureSensor + HumiditySensor,
    P: ProximitySensor + Send,
{
    ...
    if let Some(config_id) = config.id {
        ...
        id = client
            .assign_id()
            .await
            .describe_error("Failed to assign ID")?;
        config.id = Some(id);
        config
            .write_default()
            .describe_error("Failed to write config")?;
    }

    let (client_readings_in, client_readings_out) = mpsc::channel(1);
    let mut sensor_stream =
        tokio::spawn(async move { client.stream_sensor_data(client_readings_out).await });
    ...
    loop {
        ...
        match sensor_stream.get_result().await {
            ...
            Some(Err(join_error)) => {
                return Err(Error::new(join_error).context("Error checking on sensor stream"))
            }
            None => {}
        }
        ...
        if let Err(_) = client_readings_in.send(reading).await {
