use super::{HumiditySensor, Result, TemperatureSensor};
...
impl SI7021<LinuxI2CDevice> {
    pub fn new_from_descriptor<P: AsRef<Path>>(
        path: P,
        slave_address: u16,
    ) -> result::Result<Self, LinuxI2CError> {
...
impl<D: I2CDevice + Send> TemperatureSensor for SI7021<D> {
    async fn read_temperature(&mut self) -> Result<f32> {
        Ok(175.72
            * self
                .read_register_delayed(MEASURE_TEMPERATURE_NO_HOLD)
                .await? as f32
            / 65536.0
            - 46.85)
    }
...
impl<D: I2CDevice + Send> HumiditySensor for SI7021<D> {
    async fn read_humidity(&mut self) -> Result<f32> {
        Ok(
            125.0 * self.read_register_delayed(MEASURE_HUMIDITY_NO_HOLD).await? as f32 / 65536.0
                - 6.0,
        )
    }
}
...
mod tests {
    ...
    async fn test_sensor_driver() {
        ...
        device
            .regmap
            .write_regs(MEASURE_TEMPERATURE_NO_HOLD as usize, &[0x68, 0xAD]);
        device
            .regmap
            .write_regs(MEASURE_HUMIDITY_NO_HOLD as usize, &[0x68, 0x73]);
